...
class PPO(object):
    ...
    def train(self):
        ...
        for _ in range(self.K_epochs):
            ...
            s, a_r, a_f, a_p, td_target_f, td_target_p, adv_f, adv_p, old_log_prob_f, old_log_prob_p = \
                s[perm].clone(), a_r[perm].clone(), a_f[perm].clone(), a_p[perm].clone(), td_target_f[perm].clone(), td_target_p[perm].clone(),\
                adv_f[perm].clone(), adv_p[perm].clone(), old_log_prob_f[perm].clone(), old_log_prob_p[perm].clone()
            ...
            for i in range(c_optim_iter_num):
                ...
                # (bs,r_dim)
                c_loss = (self.critic(s[index]) - td_target_f[index]).pow(2).mean() + (self.critic(s[index]) - td_target_p[index]).pow(2).mean()
                value_loss += c_loss
        ...
        return 0, policy_f_loss, policy_p_loss, value_loss, 0, reward_c_loss
