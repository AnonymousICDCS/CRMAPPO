...
parser.add_argument('--user_num', type=int, default=8, help='the number of users')
...
def evaluate_policy(env, model, f_max, p_max, render):
    ...
    for j in range(turns):
        ...
        while not done:
            ...
            s_f = np.append(s, a_r)
            a_f = [f_max / env.user_num for _ in range(env.user_num)]
            a_p = [p_max / env.user_num for _ in range(env.user_num)]
            #act_f = Action_adapter(a_f, f_max)
            s_p = np.append(s_f, a_f)
            #act_p = Action_adapter(a_p, p_max)
            end = time.perf_counter()
            s_prime, r_r, r_f, r_p, done, info, fps, ave_consecutive, ave_copying, resol_score, resol_change = env.step(a_r, a_f, a_p)
...
def main():
    ...
    while total_steps < Max_train_steps:
        ...
        while not done:
            ...
            a_r, pi_r = model.select_action(torch.from_numpy(s).float().to(device), 'resolution')
            ...
            #act_p = Action_adapter(a_p, p_max)
            s_prime, r_r, r_f, r_p, done, info, fps, ave_consecutive, ave_copying, ave_resol, resol_change = env.step(a_r, a_f, a_p)
